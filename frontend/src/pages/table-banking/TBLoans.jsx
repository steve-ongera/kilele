...
import { tableBankingAPI, loansAPI, toArray } from '../../services/api'
...
export default function TBLoans() {
  const { user } = useAuth()
  ...
  const fetchLoans = async () => {
    setLoading(true)
    try {
      const res = await tableBankingAPI.loans({ status: statusFilter })
  ...
  return (
    <div>
      <div className="page-header">
        <h1 className="page-header__title">Table Banking Loans</h1>
        <p className="page-header__sub">Group lending fund loan applications and repayments</p>
          ...
          <table>
            ...
            <tbody>
              {loading ? (
                ...
                <tr><td colSpan={8}><div className="table-empty"><i className="bi bi-cash-stack" /><p>No loans found.</p></div></td></tr>
              ) : (
                loans.map(loan => (
                  <tr key={loan.id}>
              ...
              <div className="modal__body">
                ...
                <div className="form-group">
                  ...
                  <select className="form-control" value={form.product} onChange={e => setForm({ ...form, product: e.target.value })} required>
                    ...
                    {products.map(p => <option key={p.id} value={p.id}>{p.name} ({p.duration_months}m @ {p.interest_rate}%)</option>)}
                  </select>
                </div>
                <div className="form-group">
                  <label className="form-label form-label--required">Principal Amount</label>
                  <input type="number" className="form-control" value={form.principal} onChange={e => setForm({ ...form, principal: e.target.value })} required />
                </div>
                <div className="form-group">

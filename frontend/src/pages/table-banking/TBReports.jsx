...
const REPORTS = [
  { type: 'contributions', label: 'Collection Report', icon: 'bi-wallet2', desc: 'Monthly contribution collections' },
  { type: 'loans', label: 'Loans Report', icon: 'bi-cash-stack', desc: 'Lending fund loan portfolio' },
  { type: 'arrears', label: 'Arrears Report', icon: 'bi-exclamation-triangle', desc: 'Members with outstanding arrears' },
  { type: 'members', label: 'Members Report', icon: 'bi-people-fill', desc: 'Full table banking roster' },
]
...
export default function TBReports() {
  ...
  const handleGenerate = async (type, format) => {
    ...
    try {
      if (format === 'csv') {
        await reportsAPI.downloadCSV(type, filters, `table-banking-${type}-${filters.year}.csv`)
      } else {
        ...
        a.href = url
        a.download = `table-banking-${type}-${filters.year}.json`
        a.click()
  ...
  return (
    <div>
      <div className="page-header">
        ...
        <p className="page-header__sub">Generate and export Table Banking reports</p>
      </div>
